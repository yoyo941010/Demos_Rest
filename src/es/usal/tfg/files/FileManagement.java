 ...
 * la petición de descarga y la descarga en si.
 */
@Path("/files")
public class FileManagement {
	...
	/**
	 * Subida de fotografías de DNI a partir de un multipart/form-data con los
	 * siguientes campos. 
	 *  <ul>
	 ...
	 * o fracaso de la operación de subida
	 * @see <a href=
	...
	public Response download(@QueryParam("campaign") String campaignName64, @QueryParam("download_token") String downloadToken){
		...
		if (!pdfFuture.isDone()) {
			...
			response = String.format(webPageMessage, title, h1, mensaje);
			System.out.println("[" + new Date().toString() + "] Download "+campaignName+": descarga aún no preparada");
